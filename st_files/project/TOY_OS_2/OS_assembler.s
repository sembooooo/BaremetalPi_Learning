...
SysTick_Handler
	...
	LDR R0,=stackpointer
	STR SP,[R0] 	; store the current the MAIN STACK POINTER into the variable
	MRS R0, PSP
	MSR MSP,R0  	; Move the psp value into the MSP 
	...
	ORR R14,R14, #0X04  ; this i took from ucos code
	MRS R0,MSP ;
	MSR PSP,R0 ;  Store the current position of stack from MSP to PSP 
	LDR R0,=stackpointer
	LDR R0,[R0]
	MSR MSP,R0       ; Make the MSP point to the original position
